...
class EnneagramModel(QObject):
    ...
    mainTypeChanged: ClassVar[pyqtSignal] = pyqtSignal()
    wingChanged: ClassVar[pyqtSignal] = pyqtSignal()
    instinctualVariantChanged: ClassVar[pyqtSignal] = pyqtSignal()
    developmentLevelChanged: ClassVar[pyqtSignal] = pyqtSignal()
    integrationPointChanged: ClassVar[pyqtSignal] = pyqtSignal()
    disintegrationPointChanged: ClassVar[pyqtSignal] = pyqtSignal()
    wingNotationChanged: ClassVar[pyqtSignal] = pyqtSignal()

    _profile = field(init=False, default_factory=lambda: EnneagramProfile())
    ...
    def mainType(self, value: int) -> None:
        ...
        if EnneagramType.TYPE_1.value <= value <= EnneagramType.TYPE_9.value:
            ...
            if self._profile.main_type != new_type:
                ...
                self._profile.disintegration_point = (
                    self._profile.get_disintegration_point()
                )
    ...
    def instinctualVariant(self) -> str:
        ...
        return (
            self._profile.instinctual_stack[0].value
            if self._profile.instinctual_stack
            else "sp"
        )
    ...
    def instinctualVariant(self, value: str) -> None:
        ...
        try:
            ...
            if (
                not self._profile.instinctual_stack
                or self._profile.instinctual_stack[0] != new_variant
            ):
