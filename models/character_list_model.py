...
class CharacterListModel(QAbstractListModel):
    ...
    # Signals
    characterSelected: ClassVar[pyqtSignal] = pyqtSignal(str)  # Emit character ID
    countChanged: ClassVar[pyqtSignal] = pyqtSignal()  # For count property

    _characters: List[CharacterModel] = field(default_factory=list)
    _selected_index: int = field(default=-1)
    _filter_text: str = field(default="")
    ...
    def roleNames(self) -> dict:
        ...
        return {
            CharacterRoleConstant.NameRole: b"name",
    ...
    def update_character(self, character: CharacterModel):
        ...
        for i, existing_character in enumerate(self._characters):
            existing_id = (
                ...
                if hasattr(existing_character, "id")
                else str(id(existing_character))
            )
